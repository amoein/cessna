...
-record(state, {option :: option(),
                auto :: boolean(),
                open_connection = 0 :: integer(),
                workers = #{},
                lisener_socket :: [inet:socket()]}).

-record(worker_state, {count = 0 :: integer(),
                       rate = 0 :: float(),
...
init([#option{ips = []} = Option]) ->
    #option{type = Type,
            ...
            number_of_worker = WNum} = Option,

    {ok, Socket} = case Type of
                       tcp -> listen_tcp(Port, SocketOpts);
                       ssl -> listen_ssl(Port, SocketOpts);
                       _ -> error(undefinde_protocol)
                   end,
    ...
    {ok, #state{option = Option,
                lisener_socket = [Socket]}};
...
init([#option{ips = Ips} = Option]) ->
    ...
    Sockets =
        case Type of
            tcp -> [listen_tcp(Port, Ip, SocketOpts) || Ip <- Ips];
            ssl -> [listen_ssl(Port, Ip, SocketOpts) || Ip <- Ips];
            _ -> error(undefinde_protocol)
        end,

    if Auto == true -> [add_new_worker(Type, Socket, Option) || Socket <- Sockets];
       true ->
...
handle_cast({worker_start, PID}, #state{workers = WRS} = State) ->
    {noreply,
     State#state{workers = maps:put(PID, #worker_state{}, WRS)}};
...
handle_cast({notify, PID}, #state{open_connection = OC, workers = WRS} = State) ->
    #worker_state{count = Count, rate = Rate, last = Last} = maps:get(PID, WRS, #worker_state{}),

    %%TODO :auto balance worker
    %% check time for accept rate per time

    NewRate = ((?NOW() - Last) / 1000),
    NewTilt = if Rate == NewRate -> none;Rate < NewRate -> up; Rate > NewRate -> down end,
    NewWorkerState =
    ...
    NewState = State#state{open_connection = (OC + State#state.option#option.notify_pool_per_accept),
